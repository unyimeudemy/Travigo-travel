...
const Footer = ({ footerAPI: { titles, links, sociallinks } }) => {
  return (
    <>
      <footer
        className="bg-gradient-to-b from-emerald-400 to-green-300
      pt-24 pb-7 "
      >
        <div className="grid grid-cols-3 items-center justify-items-center">
          {titles?.map((val, i) => (
            <div key={i} className=" grid items-center justify-items-center">
              <h1
                className="text-xl lg:text-base uppercase 
              font-semibold"
              >
                {val.title}
              </h1>
          ...
          {links?.map((list, i) => (
            ...
            >
              {list?.map((val, i) => (
                <li key={i} className="font-medium text-sm sm:text-xsm">
                  {val.link}
                </li>
              ))}
            </ul>
        ...
        <div className="w-7/12 lg:w-[95vw] m-auto mt-9">
          <div
            className="h-[0.1vh] bg-black/30 my-7
          md:my-5"
          ></div>
          <div
            className="flex items-center justify-between px-7 md:px-0 
          md:gap-5 md:flex-col-reverse"
          >
            <p className="text:sm md:text-center">
              Copyright <sup className="text-base font-bold">&copy; </sup>
              All Reserved Right 2023
              <span className="font-semibold"> MYSTACK DEVELOPERS</span>
            </p>
